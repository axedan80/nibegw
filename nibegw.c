...
//int checkMessage(unsigned char *message, int len); - denna rad verkar finnas pÃ¥ tvÃ¥ stÃ¤llen, se rad 333
...
int initSerialPort(int fd, int hwflowctrl) {
    ...
    // FÃ¥ aktuella instÃ¤llningar
    if (tcgetattr(fd, &options) < 0) {
        perror("Failed to get serial port attributes");
        return -1;
    }

    // StÃ¤ll in hastighet (9600 baudrate hÃ¤r som exempel, nibepi kör B19200)
...
// ny kod
//readData definition (innehÃ¥lls debug-utskrifter)
ssize_t readData(int fildes, void *buf, size_t nbyte)
{
    if (testmode)
    {
